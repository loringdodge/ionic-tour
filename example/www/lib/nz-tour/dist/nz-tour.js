    ...
    module.directive('nzTour', ["$q", "$timeout", "$window", function($q, $timeout, $window) {
        return {
            ...
            link: function($scope, el, attrs) {
                ...
                // els.wrap[0].style.webkitTransform = els.wrap[0].style.webkitTransform = 'translate3d(0,0,0)';
                ...
                function moveBox() {
                    ...
                    function bottom() {
                        ...
                        if (dims.target.margins.offset.fromBottom > maxHeight) {
                            console.log(dims.target.offset.fromRight)
                            ...
                            if(dims.target.offset.fromRight + maxWidth > dims.window.width) {
                                placeVertically('bottom', 'center');
                                return true;
                            }
                            // Right, I guess...
                            console.log('vert bottom right');
                    ...
                    function right() {
                        ...
                        if (dims.target.margins.offset.fromRight > maxWidth) {
                            ...
                            // Is Element to Large to fit?
                            if (dims.target.margins.height > dims.scroll.height) {
                                console.log(dims.target.margins.height);
                                console.log(dims.scroll.height);
                            ...
                            console.log('right bottom default');
                            placeHorizontally('right', 'bottom');
                            return true;
                        }
                        return false;
                ...
                function moveMasks() {
                    ...
                    els.masks_bottom.css({
                        // Let the browser round the height by adding 0.5
                        // If not, a 1px white gap may show above the white mask
                        height: (dims.target.offset.fromBottom + 0.5) + 'px',
                        top: dims.target.offset.fromBottom < 0 ? dims.target.offset.fromBottom + 'px' : 0
                    });
                    els.masks_left.css({
